...
TInterface::TInterface(QWidget *parent)
    ...
    // для ввода нового значения корня
    (new QLabel("на значение:", this))->setGeometry(280, 110, 70, 25);
    changeRootReal = new QLineEdit("0", this);
    changeRootReal->setGeometry(350, 110, 40, 25);
    (new QLabel("+ i*", this))->setGeometry(400, 110, 30, 25);
    changeRootImag = new QLineEdit("0", this);
    changeRootImag->setGeometry(430, 110, 40, 25);

    (new QLabel("Новая размерность:", this))->setGeometry(50, 140, 120, 25);
    resizeInput = new QLineEdit("0", this);
    resizeInput->setGeometry(180, 140, 50, 25);
    resizeButton = new QPushButton("Изменить размерность", this);
    resizeButton->setGeometry(240, 140, 150, 25);
...
void TInterface::changeRootFunc() {
    ...
    bool ok1, ok2;
    double re = changeRootReal->text().toDouble(&ok1);
    double im = changeRootImag->text().toDouble(&ok2);
    if (!ok1 || !ok2) {
        output->setText("Ошибка: неверный ввод корня");
        return;
    }
    ...
    if (index >= rootsEntered) {
        rootsEntered = index + 1;
        rootsCounter->setText(QString("Корней: %1/%2").arg(rootsEntered).arg(currentDegree));
    }
    print();
}
...
void TInterface::resizePolynom() {
    bool ok;
    int newDegree = resizeInput->text().toInt(&ok);
    if (!ok || newDegree < 0) {
        output->setText("Ошибка: неверная степень");
        return;
    }
    ...
    degreeInput->setText(QString::number(currentDegree));
    rootsCounter->setText(QString("Корней: %1/%2").arg(rootsEntered).arg(currentDegree));
    print();
}
